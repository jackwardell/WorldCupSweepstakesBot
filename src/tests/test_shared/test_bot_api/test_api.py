...
@pytest.mark.parametrize(
    "input_football_fixtures, output_fixtures",
    [
        (
            [
                FootballFixture(
            ...
            [
                Fixture(
                    football_api_id=855747,
                    home_team=Team(name="1569", football_api_id=1569),
                    away_team=Team(name="13", football_api_id=13),
                    home_team_goals=1,
                    away_team_goals=3,
                    home_team_winner=False,
                    away_team_winner=True,
        ...
        (
            ...
            [
                Fixture(
                    football_api_id=866682,
                    home_team=Team(name="767", football_api_id=767),
                    away_team=Team(name="22", football_api_id=22),
                    home_team_goals=0,
                    away_team_goals=2,
                    home_team_winner=False,
                    away_team_winner=True,
...
def test_save_or_update_and_get_fixtures(
    bot_api: BotApi,
    input_football_fixtures: List[FootballFixture],
    output_fixtures: List[Fixture],
) -> None:
    for input_football_fixture in input_football_fixtures:
        bot_api.save_team(
            FootballTeam(
                football_api_id=input_football_fixture.home_team_football_api_id,
                name=f"{input_football_fixture.home_team_football_api_id}",
            )
        )
        bot_api.save_team(
            FootballTeam(
                football_api_id=input_football_fixture.home_team_football_api_id,
                name=f"{input_football_fixture.away_team_football_api_id}",
            )
        )
        bot_api.save_or_update_fixture(input_football_fixture)
    assert output_fixtures == bot_api.get_fixtures()
